...
var urlParser = {
  ...
  parse: function (url) {
    ...
    if (url.startsWith('rninedesktop:') && !url.startsWith('rninedesktop://app/')) {
      // convert shortened min:// urls to full ones
      const urlChunks = url.split('?')[0].replace(/rninedesktop:(\/\/)?/g, '').split('/')
      const query = url.split('?')[1]
      return 'rninedesktop://app/pages/' + urlChunks[0] + (urlChunks[1] ? urlChunks.slice(1).join('/') : '/index.html') + (query ? '?' + query : '')
    }
  ...
  isInternalURL: function (url) {
    return url.startsWith('rninedesktop://')
  },
    ...
    if (urlParser.isInternalURL(url)) {
      ...
      if (representedURL) {
        ...
        try {
          ...
          var urlObj = new URL(url)
          if (pageName) {
            return 'rninedesktop://' + pageName[1] + urlObj.search
          }
        } catch (e) {}
      }
    }
